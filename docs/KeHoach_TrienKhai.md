...
- [X] Tạo project Supabase + copy API keys
- [X] Khởi tạo Vite + React (`npx create-vite client --template react`)
- [X] Khởi tạo Express server (`npm init` trong folder server)
- [X] Cài dependencies (react-router, react-icons, cors, dotenv, pg)
- [X] Tạo `.env` cho cả client và server + `.gitignore`
- [X] Viết `schema.sql` (11 bảng) + `seed.sql` (data mẫu)
- [X] Chạy DDL trên Supabase SQL Editor
- [X] Test kết nối DB từ Express
...
- [X] Tạo Design System CSS (variables, base styles, typography)
- [X] Tạo Logo component
- [X] Tạo MobileLayout + DesktopLayout
- [X] Tạo UI components (Button, Input, Card, Modal, Badge)
- [X] Backend: route `/api/auth/login` + middleware JWT
- [X] Frontend: trang Login + AuthContext + ProtectedRoute
- [X] Test: login với 4 vai trò (admin/thungan/phucvu/barista)
...
- [X] Backend: CRUD giỏ hàng + tạo order
- [X] Frontend: trang CustomerMenu (card grid, filter)
- [X] Frontend: trang Cart (thêm/sửa/xóa, tổng tiền)
- [X] Frontend: trang OrderStatus (auto-refresh)
- [X] Tạo folder `public/images/menu/` + ảnh placeholder
- [X] Test luồng: quét QR → chọn món → gửi order
...
- [X] Backend: API lấy orders + cập nhật trạng thái
- [X] Frontend: trang OrderBoard (card list, trạng thái tabs)
- [X] Nút chuyển trạng thái với xác nhận
- [X] Auto-refresh 5s cho order board
- [X] Test: tạo order từ khách → nhìn thấy trên board → cập nhật
...
- [X] Backend: CRUD invoices + payments + merge
- [X] Backend: CRUD items + tables + staff
- [X] Frontend: trang Payment (2 panel layout)
- [X] Frontend: trang InvoiceList (bảng + filter + modal chi tiết)
- [X] Frontend: trang MenuMgmt (bảng CRUD + upload ảnh)
- [X] Frontend: trang TableMgmt (grid card + CRUD)
- [X] Frontend: trang StaffMgmt (bảng CRUD + tài khoản)
- [X] Test: luồng thanh toán end-to-end
- [X] Test: CRUD menu/bàn/nhân viên
